...
| sensitivity analysis exclusion | share of the reference colon beyond 60 mm > 0.05 | A | **Changed 2026-09-12.** Excludes series whose path did not reach the whole colon, by definition rather than by a tuned cut. The former coverage floor of 0.6 relied on a gap in the old, truncated coverage metric that the whole-colon metric does not have. Series without a reference are kept. |
