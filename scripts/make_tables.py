...
PILLAR1 = [
    ...
    ("Centerline length / whole-colon reference length", "length_ratio", 3),
    ("Whole-colon reference centerline within 10 mm", "ref_covered_frac", 3),
]
...
def main() -> None:
    ...
    for label, col, digits in PILLAR1:
        rows.append({"metric": label, **{k: med_iqr(d[col], digits) for k, d in ev.items()}})
    ...
    N["pillar1"]["seg_failures"] = {k: int((d["dice_lumen_vs_gas"] < SEG_FAILURE_DICE).sum())
                                    for k, d in ev.items()}
    N["pillar1"]["reference_centerline_mm"] = stats(auto["ref_centerline_len_mm"])
    ...
    N["pillar1"]["colon_reached"] = int((auto["colon_beyond_60mm_frac"] < REACHED_FAR_MAX).sum())
    ...
    for label, d in ev.items():
        N["pillar1"].setdefault("by_configuration", {})[label] = {
            ...
            "colon_reached": int((d["colon_beyond_60mm_frac"] < REACHED_FAR_MAX).sum()),
    ...
    hc = pd.read_csv(t / "prone_supine_agreement_reached.csv").set_index("index")
    rows = []
    for construct, idx, label, unit, d in AGREEMENT:
        r, h = agr.loc[idx], hc.loc[idx]
        rows.append({
            ...
            "ICC(2,1), whole colon reached": f"{h.icc21:.3f} [{h.icc_lower:.3f}, "
                                             f"{h.icc_upper:.3f}]",
        })
    ...
    N["asymmetry_definitions"] = {
        k: {"icc21": round(float(agr.loc[k, "icc21"]), 3),
            ...
            "icc21_reached": round(float(hc.loc[k, "icc21"]), 3),
            "ci_reached": [round(float(hc.loc[k, "icc_lower"]), 3),
                           round(float(hc.loc[k, "icc_upper"]), 3)]}
    ...
    N["agreement"] = {"n_pairs": int(agr["n_pairs"].iloc[0]),
                      "n_pairs_reached": int(hc["n_pairs"].iloc[0]),
